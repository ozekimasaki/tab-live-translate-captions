(() => {
  ...
  var DeepframPcmRecorderProcessor = class extends AudioWorkletProcessor {
    constructor(options) {
      super();
      this.targetSampleRate = options.processorOptions?.targetSampleRate || 16e3;
    ...
    process(inputs, outputs) {
      ...
      while (this.pending.length >= this.chunkSize) {
        ...
        const chunkDurationMs = Math.round(buffer.length / this.targetSampleRate * 1e3);
    ...
    calculateRms(buffer) {
      ...
      for (let index = 0; index < buffer.length; index += 1) {
        const normalized = buffer[index] / 32767;
